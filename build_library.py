...
def PolyDiffPoint(u, x, deg=3, diff=1, index=None):
    ...
    poly, list = np.polynomial.chebyshev.Chebyshev.fit(x, u, deg, full=True)
                ...
                for j in range(d):
                    if rhs_functions[k][1][j] != 0:
                        if rhs_functions[k][1][j] == 1:
                            ...
                            function_description = function_description + data_description[j] + '^' + str(
                                rhs_functions[k][1][j])
...
def build_linear_system(u, dt, dx, D=3, P=3, time_diff='poly', space_diff='poly', lam_t=None, lam_x=None, width_x=None,
                        width_t=None, deg_x=5, deg_t=None, sigma=2):
    """
    ...
    for d in range(D + 1):
        ...
        for p in range(P + 1):
            ...
            if p == 1:
                rhs_description[d * (P + 1) + p] = rhs_description[d * (P + 1) + p] + 'u'
            elif p > 1:
                rhs_description[d * (P + 1) + p] = rhs_description[d * (P + 1) + p] + 'u^' + str(p)
